...
for(target_var in dly_vars){
  ...
  for(k in laks){
    ...
    df <- ddply(df2[idx,], c('year', 'lake', 'scenario', 'model', 'variable'), .fun = function(x){
      mean(x$anom)
    })
...
summary(out)
colnames(out) <- c('Year', 'Lake','Scenario', 'GCM',  'variable','anom')
...
mlt$disc <- cut(mlt$value, seq(-0.6,0.6,0.2), include.lowest = T)
levels(mlt$disc) <- seq(-0.5,0.5,0.2)
...
ggsave('C:\\Users\\mooret\\Dropbox\\ISIMIP_chapter\\fig_v3/all_lake_year_density_difference_RCP60_anomaly+SD_v1.png', p2, dpi = 300,width = 270,height = 297, units = 'mm')
...
mlt$disc <- cut(mlt$value, seq(-1,1,0.2), include.lowest = T)
...
p2 <- ggplot(mlt, aes(Month, label))+
  geom_tile(aes(fill = (disc)))+
  scale_fill_manual(values = rev(incol), name = expression(~ Delta * rho~ "(kg"~m^-3*")"), drop = F, na.value = 'grey')+
  facet_wrap(~variable, ncol = 2)+
  xlab('Month')+
  ylab('')+
  scale_y_discrete()+
...
ggsave('C:\\Users\\mooret\\Dropbox\\ISIMIP_chapter\\fig_v3/all_lake_month_density_difference_RCP60_anomaly+SD_2069-99_v1.png', p2, dpi = 300,width = 270,height = 297, units = 'mm')
