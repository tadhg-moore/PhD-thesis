...
df3$variable <- factor(df3$variable, levels = levels(df3$variable)[c(3,2,1)])
levels(df3$variable) <- c('Stratification duration','Stratification start', 'Stratification end')
...
p1 <- ggplot(df3[idx,], aes(Year, mean))+
  ...
  theme(strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA),
        ...
        legend.text = element_text(size = 16))
p1
ggsave('C:\\Users\\mooret\\Dropbox\\ISIMIP_chapter/fig_v3/mean_lake-GCM_StratStart_StratEnd_TotStratDur_RCP_line_v3.png', p1, dpi = 300,width = 310,height = 220, units = 'mm')
