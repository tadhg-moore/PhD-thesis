...
sep <- data.frame(y = c(5,6,9,20,24,25,35)-0.5)
xmx <- 13.3
xcex <- 2.1
...
p1 <- ggplot(mlt, aes(Month, label))+
  # geom_tile(aes(fill = sd))+
  geom_tile(aes(fill = (disc)))+
  scale_fill_manual(values = rev(incol), name = '\u00B0C', drop = T)+
  # scale_fill_gradientn(colours = rev(cols), name = 'Anomaly (\u00B0C)', limits = c(-4,4), breaks = MakeBreaks(1))+
  facet_wrap(~variable, ncol = 2)+
  xlab('Month')+
  ylab('')+
  scale_y_discrete()+
  scale_x_continuous(breaks = seq(1,12,2), labels = month.abb[seq(1,12,2)])+  # scale_y_continuous(breaks = seq(-2, 8,1))+
  labs(tag = 'A')+
  ...
  annotate('text', x = xmx, y = 40, label = 'NEU', size = xcex)+
  annotate('text', x = xmx, y = 29, label = 'CEU', size = xcex)+
  annotate('text', x = xmx, y = 24, label = 'MED', size = xcex)+
  annotate('text', x = xmx, y = 21, label = 'ENA', size = xcex)+
  ...
  annotate('text', x = xmx, y = 2, label = 'SAU', size = xcex)+
...
p2 <- ggplot(mlt, aes(Month, label))+
  # geom_tile(aes(fill = sd))+
  geom_tile(aes(fill = (disc)))+
  scale_fill_manual(values = rev(incol), name = expression(~m ~s^-1), drop = T)+
  # scale_fill_gradientn(colours = rev(cols), name = 'Anomaly (\u00B0C)', limits = c(-4,4), breaks = MakeBreaks(1))+
  facet_wrap(~variable, ncol = 2)+
  xlab('Month')+
  ylab('')+
...
p3 <- ggplot(mlt, aes(Month, label))+
  # geom_tile(aes(fill = sd))+
  geom_tile(aes(fill = (disc)))+
  scale_fill_manual(values = rev(incol), name = expression(~W ~m^-2), drop = T)+
  # scale_fill_gradientn(colours = rev(cols), name = 'Anomaly (\u00B0C)', limits = c(-4,4), breaks = MakeBreaks(1))+
  facet_wrap(~variable, ncol = 2)+
  xlab('Month')+
  ylab('')+
...
# 
# ggsave('C:\\Users\\mooret\\Dropbox\\ISIMIP_chapter\\fig_v2/wtemp_obs_month_stripe_density_diff_Region_v1.png', p2, dpi = 300,width = 210,height = 270, units = 'mm')
# 
# ggsave('analysis-isimip/plots/all_lake_month_tas_sfcWind_rsds_RCP60_month_anomaly+SD_2069-99_v4.png', g1, dpi = 300,width = 210,height = 297, units = 'mm')
# library(grid)
# library(gtable)
...
# grid.newpage()
# grid.draw(g)
ggsave('C:\\Users\\mooret\\Dropbox\\ISIMIP_chapter\\fig_v2/all_lake_month_tas_sfcWind_rsds_RCP60_anomaly+SD_2069-99_v1.png', g, dpi = 300,width = 210,height = 270, units = 'mm')

# ggsave('analysis-isimip/plots/all_lake_month_tas_sfcWind_rsds_RCP60_month_anomaly+SD_2069-99_v4.png', g, dpi = 300,width = 210,height = 297, units = 'mm')
...
# g1 <- ggarrange(p1,p2,p3, nrow = 3, labels = 'AUTO', align = 'hv')
# g1 <- gridExtra::grid.arrange(p1, p2, p3, )
# ggsave('analysis-isimip/plots/all_lake_year_tas_sfcWind_rsds_RCP60_year_anomaly+SD_2069-99_v4.png', g1, dpi = 300,width = 210,height = 297, units = 'mm')
library(grid)
library(gtable)
...
# grid.newpage()
# grid.draw(g)
ggsave('C:\\Users\\mooret\\Dropbox\\ISIMIP_chapter\\fig_v2/all_lake_year_tas_sfcWind_rsds_RCP60_anomaly+SD_2069-99_v1.png', g, dpi = 300,width = 210,height = 297, units = 'mm')
...
p1 <- p
p1
ggsave('analysis-isimip/plots/met_tas_pairs_plot.png', p1, dpi = 300,width = 340,height = 340, units = 'mm')
...
p2 <- p
p2
ggsave('analysis-isimip/plots/met_sfcWind_pairs_plot.png', p2, dpi = 300,width = 340,height = 340, units = 'mm')
...
p3 <- p
p3
ggsave('analysis-isimip/plots/met_rsds_pairs_plot.png', p3, dpi = 300,width = 340,height = 340, units = 'mm')
