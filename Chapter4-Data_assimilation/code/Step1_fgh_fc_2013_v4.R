setwd("C:\\Users\\mooret\\OneDrive - Dundalk Institute of Technology\\PROGNOS_offline\\Met_Comparison\\feeagh")
...
library(grid)
library(MASS)
# source("C:\\Users\\mooret\\Documents\\GOTMr\\gotmtools\\R\\get_param.R")

fc.days = 14
fcstart = '2013-01-01 23:00:00' #Need 23:00 for restart function
fcend = '2014-01-14'
spin_start = '2012-01-01 00:00:00'
...
out_file = 'Output/opt_freq/run3/fc_3day_2012_v1' #Leave out .csv
num = 1 #Always leave at 1
par_ens <- TRUE
par_runs <- 100

#Load in observed Lake data
obs.file = 'acpy/Lev3_wtemp/obs_1hr_lev3_adj.dat'
docb.file = 'acpy/feeagh_doc_watras_correct.obs'
out = 'output.nc'
rest = 'restart.nc'
rest_temp_file = 'restart_temp.nc'
obs_wtemp = load_obs(obs.file)
obs_wtemp <- obs_wtemp[(obs_wtemp$date >= (as.POSIXct(fcstart) - max(rest_step)*1.5*60*60) & obs_wtemp$date < (as.POSIXct(fcend) + 15*24*60*60)),]
colnames(obs_wtemp)[3] <- 'obs'

fgh.docb = load_obs(docb.file, sep = ' ')

#Load in calibration parameter data
db <- 'acpy/Rd3/rd3_feeagh_wnd_obs_hr.db'
acpyXML <- 'acpy/rd3_config_wind.xml'
...
par.nams <- colnames(ens_pars)
ens_pars$id <- c('cal',1:par_runs)
write.csv(ens_pars, 'Output/opt_freq/run3/ens_parameters_v1.csv', row.names = F, quote = F)
...
g1
ggsave(paste0(out_file,'_par_dist.png'), g1, dpi = 300,width = 384,height = 216, units = 'mm')			
...
input_nml(val = 'met_files/met_local_obs_hr.dat', nml = 'airsea', par = 'meteo_file')
input_nml(val = 'met_swr_file.dat', nml = 'airsea', par = 'swr_file')
input_nml(val = 'met_precip_file.dat', nml = 'airsea', par = 'precip_file')
input_nml(val = 3, nml = 'airsea', par = 'hum_method')
...
input_nml(val = 'zeta_level.dat', nml = 'obs.nml', par = 'zeta_file')
input_nml(val = -10.312, nml = 'obs.nml', par = 'zeta_offset')
...
wdir <- "C:\\Users\\mooret\\OneDrive - Dundalk Institute of Technology\\PROGNOS_offline\\Met_Comparison\\feeagh\\"
met_dir <- "C:\\Users\\mooret\\OneDrive - Dundalk Institute of Technology\\PROGNOS_offline\\Met_Comparison\\feeagh\\met_files\\"
catchdir <- "C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\catchment_model\\"
length(rest_step)*nrow(ens_pars)*length(fc_init)
#fc_dir <- "C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\forecast_files\\"
...
for(i in 1:length(fc_init)){ #length(fc_init)
  ...
  # 
  # 
  # grid.arrange(wnd,mslp,temp,cc, top = textGrob(paste("Lough Feeagh - Yr Forecast at ", fc.date),gp=gpar(fontsize=15,font=3)))
  ...
  if(catchment_model == TRUE){
    ...
    setwd('C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\catchment_model\\Black')
    
    #run PERSiST
    system('C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\catchment_bin\\persist_cmd.exe -par persist_met_black.par -dat persist_met_fc.dat -inca black_INCA_fc.dat -out black')
    ...
    #run INCA-C
    system('C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\catchment_bin\\inca_c_cmd.exe -par black_INCA_fc.par -dat black_INCA_fc.dat -out black_results.dsd')
    ...
    write.table(docb, 'C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\black_mod_domb_INCA.dat', row.names = F, col.names = F, quote = F, sep = '\t')
    
    #Format modelled flow for GOTM
    flo <- mod[,c('Date', 'Flow','Water temperature')]
    flo$Date <- format(flo$Date, "%Y-%m-%d %H:%M:%S")
    write.table(flo, 'C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\black_mod_QT_INCA.dat', row.names = F, col.names = F, quote = F, sep = '\t')
    
    ############## PERSIST Glen ######################
    
    setwd('C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\catchment_model\\Glen')
    #run PERSiST - Glen
    system('C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\catchment_bin\\persist_cmd.exe -par persist_met_glen.par -dat persist_met_fc.dat -inca glen_INCA_fc.dat -out glen')
    ...
    #run INCA-C
    system('C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\catchment_bin\\inca_c_cmd.exe -par glen_INCA_fc.par -dat glen_INCA_fc.dat -out glen_results.dsd')
    ...
    write.table(docb, 'C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\glen_mod_domb_INCA.dat', row.names = F, col.names = F, quote = F, sep = '\t')
    
    #Format modelled flow for GOTM
    flo <- mod[,c('Date', 'Flow','Water temperature')]
    flo$Date <- format(flo$Date, "%Y-%m-%d %H:%M:%S")
    write.table(flo, 'C:\\Users\\mooret\\Desktop\\Git\\PROGNOS\\feeagh\\glen_mod_QT_INCA.dat', row.names = F, col.names = F, quote = F, sep = '\t')
  ...
  for(tstep in rest_step){
    ...
    #   all_tmp <- tmp_fc1
    #   write.csv(all_tmp, out_file, row.names = F, quote = F)
    # }else{
    #   all_tmp <- rbind.data.frame(all_tmp, tmp_fc1)
    #   write.csv(all_tmp, out_file, row.names = F, quote = F)
    # }
